...
def sell(cash, owned_volume, price, percentage, date, hour, minute):
    ...
    if sell_value > 0:
        append_pine_script(date, hour, minute, "卖", sell_shares, price, 
            after_shares, after_cash, market_value)
        print("%s sell %.2f shares at %.2f. Market value = %.2f, cash = %.2f, owned_volume = %.2f." % (date, sell_value / price, price, market_value, cash + sell_value, owned_volume - sell_value / price))
    return after_cash, after_shares
...
if __name__ == "__main__":
    ...
    # Result
    print("1月1日用懒人炒股心经结果是：%.0f 美元，上涨了%.0f%%" % (best_market_value, percentIncrease))
    print_basic(data)